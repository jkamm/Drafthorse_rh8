...
namespace Drafthorse.Helper
{
    public static class Layout
    {
        ...
        /// <summary>
        /// Revised Duplicate Layout using Grasshopper Type as input
        /// </summary>
        /// <param name="template"></param>
        /// <returns></returns>
        /// <exception cref="IndexOutOfRangeException"></exception>
        public static RhinoPageView DupLayout(Grasshopper.Rhinoceros.Display.ModelPageViewport template)
        {
            RhinoPageView page;

            template.CastTo<RhinoPageView>(out page);
            //goal: if not found, Throw exception
            if (page != null)
            {
                try
                {
                    //Duplicate it
                    var dup = page.Duplicate(true);
                    return dup;
                }
                catch (IndexOutOfRangeException)
                {
                    throw new IndexOutOfRangeException("No template found with name: " + page.PageName);
                    //This may no longer be useful/necessary
                }
            }
            else throw new IndexOutOfRangeException("No template found");
        }
