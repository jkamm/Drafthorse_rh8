...
{
    public class LayoutEdit_OBSOLETE : Base.DH_ButtonComponent
    {
        /// <summary>
        /// Initializes a new instance of the LayoutEdit class.
        /// </summary>
        public LayoutEdit_OBSOLETE()
        ...
        protected override void RegisterInputParams(GH_Component.GH_InputParamManager pManager)
        {
            ...
            pManager.AddIntegerParameter("Index", "Li[]", "Index of a Layout (from LayoutIndex Drafthorse)\nAttach Value List for Layout List", GH_ParamAccess.item);
        ...
        public void InputParamChanged(IGH_DocumentObject sender, GH_ObjectChangedEventArgs e)
        {
            ...
            if (sender.NickName == Params.Input[0].NickName)
            {
                ...
                var pageList = Rhino.RhinoDoc.ActiveDoc.Views.GetPageViews();
                //var pageDictionary = Rhino.RhinoDoc.ActiveDoc.Views.GetPageViews().ToDictionary(v => v.PageNumber, v => v.PageName);
                List<string> pageViewNames = new List<string>();
                    //pageDictionary.Values.ToList();
                List<string> layoutIndices = new List<string>();
                for (int i = 0; i < pageList.Length; i++)
                {
                    pageViewNames.Add(pageList[i].PageName);
                    layoutIndices.Add(pageList[i].PageNumber.ToString());
                }
