        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            if (Execute || run)
            {
                if (pageView != null)
                {
                    ...
                    if (detail != null)
                    {
                        result = Helper.Layout.ReviseDetail(detail, targetBBox, scale, projection, displayMode, view.ToViewportInfo());
                    }

                    Grasshopper.Rhinoceros.Display.ModelView newView = new Grasshopper.Rhinoceros.Display.ModelView(new Rhino.DocObjects.ViewportInfo(detail.Viewport));
